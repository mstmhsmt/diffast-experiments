...
//		All(new BigInteger("2199023255551"));
  ...
  public static void
    feedRefactoringsInstances(BigInteger refactoringsFlag,
                              int systemsFlag,
                              TestBuilder test)
  ...
  public static List<Root>
    getFSERefactorings(BigInteger flag)
    throws JsonParseException, JsonMappingException, IOException
  {
  ...
  private static boolean isAdded(Refactoring refactoring, BigInteger flag) {
    try {
      BigInteger value =
        Enum.valueOf(Refactorings.class, refactoring.type.replace(" ", "")).getValue();
  ...
  private static String buildResultMessage(Tuple... result) {
    ...
    try {
      String mainResultMessage =
        String.format("TP: %2d  FP: %2d  FN: %2d  Unk.: %2d  Prec.: %.3f  Recall: %.3f",
